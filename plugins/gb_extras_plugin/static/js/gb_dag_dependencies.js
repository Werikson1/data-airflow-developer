...
$('#tags_filter').on('change', (e) => {
  ...
  window.location = `${dependencyIndex}?${query.toString()}`;
});

$('#tags_filter_form').on('reset', (e) => {
  e.preventDefault();
  const query = new URLSearchParams(window.location.search);
  query.delete('tags');
  window.location = `${dependencyIndex}?${query.toString()}`;
...
//Filtro por states
$('#states_filter').select2({
  placeholder: 'Filter DAGs by States'
});

$('#states_filter').on('change', (e) => {
  e.preventDefault();
  const query = new URLSearchParams(window.location.search);
  query.delete("states");
  if (e.val) query.set("states", e.val);
  window.location = `${dependencyIndex}?${query.toString()}`;
});

$('#states_filter_form').on('reset', (e) => {
  e.preventDefault();
  const query = new URLSearchParams(window.location.search);
  query.delete('states');
  window.location = `${dependencyIndex}?${query.toString()}`;
...
function setUpNodeHighlighting(focusItem = null) {
  ...
  d3.selectAll('g.node').on('click', function (nodeId) {
    console.log(nodeId);
    $('#dagModal').modal({});
