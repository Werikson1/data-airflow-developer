...
class GbExternalTaskSensor(ExternalTaskSensor):
    ...
    valid_dependency_modes = [LAST_VALID, LAST_IN_RANGE, LAST_SUCCESS_IN_RANGE]  # type: Iterable[str]

    def __init__(self,
                 *,
                 failed_states: Optional[Iterable[str]] = [],
                 execution_date_fn: Optional[Callable] = None,
                 dependency_mode: str = LAST_VALID,
                 tolerance: int = None,
                 mode="reschedule",
                 **kwargs):
        ...
        if execution_date_fn is None:
            ...
            execution_date_fn = getattr(self, self.dependency_mode)
    ...
    @provide_session
    def last_valid(self, execution_date, session=None, **kwargs):
        """
    ...
    @provide_session
    def last_in_range(self, execution_date, session=None, **kwargs):
        """
        ...
        print('*' * 50)
        print(f'Execution_date of actual dag {execution_date}')
        print(f'Period start in {start_period}')
        print(f'Execution_date of {kwargs.get("task").external_dag_id}: {last_execution_date}')
        print('*' * 50)
    ...
    @provide_session
    def last_success_in_range(self, execution_date, session=None, **kwargs):
        """
